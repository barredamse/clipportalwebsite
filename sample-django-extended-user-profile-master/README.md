...
< PROJECT ROOT >
   ...
   |
   |-- clients/                          # Handles the profile edit     <-------- NEW
...
To do this, we create a new app called `clients`, then create a model named `Profile` to store user information.
...
class Profile(models.Model):
    ...
    avatar   = models.ImageField(upload_to="clients/profiles/avatars/", null=True, blank=True)
...
Create related form to show inputs & store data. The `ProfileForm` will be defined in the `clients/forms.py` using a definition as below:
...
* create `ProfileView` in `clients/views.py`:

```python
from django.contrib import messages # import messages to show flash message in your page
from clients.forms import ProfileForm, form_validation_error # import the used form and related function to show errors
from clients.models import Profile # import the Profile Model 
...
The template that handles the user input is defined in `clients/profile.html` file. 
...
Activate the routing for `clients` app by edit the `clients/urls.py` file with the following code:
...
**Update `core/urls`** to include `clients` urls:
...
urlpatterns = [
    ...
    path('clients/', include("clients.urls")),  # Django clients route
    # ...
]
